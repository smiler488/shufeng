...
// @ts-ignore;
import { Upload, Image as ImageIcon, Download, Search, Grid, List, Star, Clock, Users, Play, RotateCcw, FileText, FileCode, Camera, X } from 'lucide-react';
// @ts-ignore;
...
export default function ImageQuantitativeAnalysis(props) {
  ...
  const canvasForCameraRef = useRef(null);
  ...
  // 拍照
  const takePhoto = () => {
    if (!videoRef.current || !canvasForCameraRef.current) return;
    const video = videoRef.current;
    const canvas = canvasForCameraRef.current;
            ...
            <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm p-6">
              ...
              {!imagePreview ? <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <button onClick={() => document.getElementById('fileInput').click()} className="flex flex-col items-center justify-center p-4 border-2 border-dashed border-gray-300 dark:border-gray-600 rounded-lg hover:border-blue-400 transition-colors">
                      <Upload className="w-8 h-8 text-gray-400 mb-2" />
                      <span className="text-sm text-gray-600 dark:text-gray-300">选择文件</span>
                    </button>
                    <button onClick={startCamera} className="flex flex-col items-center justify-center p-4 border-2 border-dashed border-gray-300 dark:border-gray-600 rounded-lg hover:border-green-400 transition-colors">
                      <Camera className="w-8 h-8 text-gray-400 mb-2" />
                      <span className="text-sm text-gray-600 dark:text-gray-300">拍照</span>
                    </button>
                  </div>
                  
                  <div className="text-center">
                    <p className="text-gray-600 dark:text-gray-300 mb-2">拖拽图像到此处或选择上传方式</p>
      ...
      {showCamera && <div className="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center z-50">
          <div className="bg-white dark:bg-gray-800 rounded-lg p-6 max-w-4xl w-full mx-4">
            <div className="flex justify-between items-center mb-4">
              <h3 className="text-lg font-semibold text-gray-900 dark:text-white">拍照</h3>
            ...
            <div className="relative">
              {cameraError ? <div className="bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-lg p-8 text-center">
                  <Camera className="w-16 h-16 text-red-400 mx-auto mb-4" />
                  <p className="text-red-600 dark:text-red-400 mb-2">{cameraError}</p>
                  <p className="text-sm text-red-500 dark:text-red-300">
                    请确保浏览器已授予相机权限，或尝试刷新页面
                  </p>
                </div> : <div className="relative">
                  <video ref={videoRef} autoPlay playsInline className="w-full rounded-lg bg-black" style={{
              maxHeight: '60vh'
            }} />
                  <div className="absolute bottom-4 left-1/2 transform -translate-x-1/2">
                    <button onClick={takePhoto} className="bg-blue-600 text-white p-4 rounded-full hover:bg-blue-700 transition-colors shadow-lg">
            ...
            <canvas ref={canvasForCameraRef} className="hidden" />
          </div>
        </div>}
